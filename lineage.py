...
def LineageOccurences(df_data, dict_lineages):
    ...
    for lineage in dict_lineages.keys():
        ...
        x_min, x_max = df_data.query(query).Day.agg([min, max]).T
        query = '@x_min <= Day <= @x_max'
        df_lineage = df_data.query(query)
        eval_ = 'paper_lineage = paper_lineage in @lineage_list'
...
def myop(obj_func, initial_theta, bounds):
    ...
    opt_res = op.minimize(obj_func, initial_theta,
                          method="L-BFGS-B", jac=True,
                          bounds=bounds, options={
                              'maxfun': 100000, # Default is 15000
                              'maxiter': 100000,} # Default is 15000
                              )
...
def GPFitting(occurences, df_counts):
    ...
    for i, lineage in enumerate(occurences.keys()):
        ...
        if (m < ncmax):
            ...
            kernel1 = 1.0 * RBF(length_scale=10.) + WhiteKernel(noise_level=1)
            gpr1 = GaussianProcessRegressor(kernel=kernel1, alpha=0.0, 
                                            n_restarts_optimizer=10, optimizer=myop)
            gpr1.fit(X0,y1)
            y_mean1 = gpr1.predict(X1, return_std=False)

            kernel2 = 1.0 * RBF(length_scale=10.) + WhiteKernel(noise_level=1.0)
            gpr2 = GaussianProcessRegressor(kernel=kernel2, alpha=0.0, 
                                            n_restarts_optimizer=10, optimizer=myop)
            gpr2.fit(X0,y2)
            ...
            Pi = (np.exp(mu2)-1)/(np.exp(mu1)+np.exp(mu2)-2)
            ...
            for j in tqdm(range(nboot)):
                ...
                gpr1 = GaussianProcessRegressor(kernel=k1store, alpha=0.0, n_restarts_optimizer=1)
                gpr1.fit(X0, y1)
                y_mean1 = gpr1.predict(X1, return_std=False)

                gpr2 = GaussianProcessRegressor(kernel=k2store, alpha=0.0, n_restarts_optimizer=1)
                gpr2.fit(X0, y2)
                y_mean2 = gpr2.predict(X1, return_std=False)

                mu1 = y_mean1.reshape(-1)
                mu2 = y_mean2.reshape(-1)
                pb[j,:] = (np.exp(mu2)-1)/(np.exp(mu1)+np.exp(mu2)-2)
        ...
        X1 = X1.reshape(-1)
        df_Pi = pd.DataFrame(index=X1, data=Pi, columns=[lineage])
        Pi_store = Pi_store.join(df_Pi, how='outer')
        dr = pd.DataFrame(index=X1[1:], data=dr, columns=[lineage])
    ...
    return {'Pi_store':Pi_store, 'Pi_boot':Pi_boot, 'r_store':r_store, 'r_boot':r_boot}
